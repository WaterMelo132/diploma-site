...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && 
    ...
    if (!empty($input['csrf_token']) && hash_equals($_SESSION['csrf_token'], $input['csrf_token']) && !empty($input['id']) && !empty($input['title']) && !empty($input['destination']) && !empty($input['price']) && !empty($input['transport_type'])) {
        ...
        if (!empty($images)) {
            $imageUrls = array_filter(array_map('trim', explode(',', $images)));
            foreach ($imageUrls as $url) {
                if (!filter_var($url, FILTER_VALIDATE_URL)) {
                    $imageUrls = array_diff($imageUrls, [$url]);
                }
            }
        ...
        $stmt = $conn->prepare("
            UPDATE travels 
            SET title = ?, destination = ?, price = ?, status = ?, start_date = ?, end_date = ?, transport_type = ?, transport_type_en = ?, image = ?, images = ?
            WHERE id = ?
        ");
        $stmt->bind_param("ssdsssssssi", $title, $destination, $price, $status, $start_date, $end_date, $transport_type, $transport_type_en, $imagePath, $images, $id);
        if ($stmt->execute()) {
            $stmt->close();
            echo json_encode([
                'success' => true,
...
// Валидация URL (опционально)
if (!empty($imagePath) && !filter_var($imagePath, FILTER_VALIDATE_URL)) {
    $imagePath = ''; // Сбрасываем, если URL недействителен
}
if (!empty($images)) {
    $imageUrls = array_filter(array_map('trim', explode(',', $images)));
    foreach ($imageUrls as $url) {
        if (!filter_var($url, FILTER_VALIDATE_URL)) {
            $imageUrls = array_diff($imageUrls, [$url]); // Удаляем недействительные URL
        }
    }
    $images = implode(',', $imageUrls);
}
                    ...
                    <div class="admin-section">
                        ...
                        <form id="add-tour-form" class="admin-form" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-row">
    <div class="form-group">
        <label for="image_url">Ссылка на основное изображение</label>
        <input type="url" id="image_url" name="image_url" placeholder="https://example.com/image.jpg">
    </div>
</div>
<div class="form-row">
    <div class="form-group">
        <label for="image_urls">Ссылки на дополнительные изображения (через запятую)</label>
        <textarea id="image_urls" name="image_urls" placeholder="https://example.com/image1.jpg, https://example.com/image2.jpg"></textarea>
    </div>
</div>
                            <div class="form-group">
                    ...
                    <div id="edit-tour-modal" class="modal" style="display: none;">
                        <div class="modal-content">
                            ...
                            <form id="edit-tour-form" class="admin-form" method="POST" enctype="multipart/form-data">
                               ...
                               <div class="form-group">
    <label for="edit_image_url">Ссылка на основное изображение</label>
    <input type="url" id="edit_image_url" name="image_url" placeholder="https://example.com/image.jpg">
</div>
<div class="form-group">
    <label for="edit_image_urls">Ссылки на дополнительные изображения (через запятую)</label>
    <textarea id="edit_image_urls" name="image_urls" placeholder="https://example.com/image1.jpg, https://example.com/image2.jpg"></textarea>
</div>
                                <div class="form-group">
...
if (addTourForm) {
    addTourForm.addEventListener('submit', function(e) {
        ...
        const data = {
            ...
            transport_type_en: this.querySelector('#transport_type option:checked')?.dataset.en || '',
        ...
        if (tourId) {
            ...
            .then(data => {
                if (data.success && data.tour) {
                    ...
                    document.getElementById('edit_image_url').value = tour.image || '';
                    document.getElementById('edit_image_urls').value = tour.images || '';
